...
Memory 窗口因地址预加载导致调试会话挂起
-------------------------------------

**问题描述**

在 IDE 中使用 Memory 窗口查看内存时，界面卡死，调试会话无响应。典型场景：查看 `0xf0030000` 地址时，IDE 自动预加载相邻地址数据，触及未映射区域（如 `0xf002xxxx`），导致目标返回访问错误。

**根本原因**

Memory 窗口的 **Automatic 渲染模式** 会在可视区域外预加载数据，以优化滚动体验。当预加载地址落入：

- 未映射的内存区域

- MPU 保护的保留区域  

- 外设地址空洞（peripheral holes）

目标硬件会返回 Bus Fault 或访问错误，GDB 进入等待/错误状态，表现为 IDE 卡死。

**解决方案**

**推荐：切换为 Manual 模式**

1. Memory 窗口右键 → **Table Renderings Preferences...**
2. Memory loading mode 选择 **Manual**
3. 点击 OK
...
.. |image64| image:: /asserts/nucleistudio/faq/image64.png

.. |image65| image:: /asserts/nucleistudio/faq/image65.png
