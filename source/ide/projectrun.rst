...
如果需要在调试过程中启用 Semihosting，请勾选本选项；也可以直接在 OpenOCD 配置文件中添加 ``arm semihosting enable`` 。关于工程配置和 J-Link 调试的更多说明，请参考：

- :ref:`如何构建支持Semihosting的工程 <faq_how_to_build_semihosting_project>`
- :ref:`如何用J-Link调试支持Semihosting的工程 <faq_debug_semihosting_project_with_jlink>`
